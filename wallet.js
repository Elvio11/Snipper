...
export function getVaultWallet() {
  if (!_vaultWallet) {
    if (CONFIG.PAPER_TRADING) {
      ...
      const secret = Buffer.from(CONFIG.PRIVATE_KEY, 'base64');
...
export function getSignerWallet() {
  if (!_signerWallet) {
    if (CONFIG.PAPER_TRADING) {
      ...
      const secret = Buffer.from(CONFIG.SIGNER_PRIVATE_KEY, 'base64');
...
export async function getSOLBalance() {
  if (CONFIG.PAPER_TRADING) return 0.059 * 10;
...
export async function getVaultBalance() {
  if (CONFIG.PAPER_TRADING) return 0.059 * 80;
  const conn = getConnection();
...
export async function getSolPrice() {
  ...
  try {
    ...
    const data = await res.json();
    return data?.data?.['So11111111111111111111111111111111111111112']?.price || 90;
  } catch (e) {
